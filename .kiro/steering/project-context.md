...
- **Juli 2026 (fix):** Data **Guns N' Roses** diupdate — harga tiket sudah tersedia di TipTip (presale Nightrain mulai Rp 1.050.000 sejak 2 Juli 2026). Perubahan: `time` 19:00→17:00 WIB, `ticketPlatform` gnrjakarta.com→TipTip, `ticketUrl` ke halaman TipTip, `priceRange` Segera diumumkan→Rp 1.050.000 — TBA, `priceMin` 0→1050000, `ticketCategories` 3 tier (Nightrain/BALE/General), `sources` tambah tiptip.id. Wajib sync ke mobile `src/data/concerts.ts`.
- **Juni 2026 (feat):** Konser **Guns N' Roses — World Tour 2026 Jakarta** dikonfirmasi. Entry GNR lama yang berstatus **rumor** (`gnr-jakarta-rumor`) **dihapus**, diganti entry **confirmed** (`guns-n-roses-jakarta-2026`): Sabtu 21 November 2026, Stadion Madya GBK, promotor Rajawali Indonesia. Sumber resmi: `gnrjakarta.com` (dedicated promo site + JSON-LD event). Gambar disimpan sebagai `images/guns-n-roses-jakarta-2026.jpeg` (poster resmi 1440×740 dari gnrjakarta.com) — **nama file WAJIB sama dengan id** karena mobile (`ConcertCard`/`HomeScreen`/`DetailScreen`) menyusun URL banner langsung dari `${concert.id}.jpeg`, bukan dari `ARTIST_IMAGES`. File lama `gnr-jakarta-rumor.jpeg` dihapus. **Total tetap 44 entries** (1 rumor diganti 1 confirmed). Wajib sync ke mobile `src/data/concerts.ts`.
...
- **Juli 2026 (seo):** `<lastmod>` dihapus dari `sitemap.xml` (4 entry). Tanggal `2026-07-26` hardcoded dan beku, padahal data konser berubah harian via scraper 01:00 WIB — jadi `lastmod` selalu bohong. Google mengabaikan `lastmod` yang tidak melacak perubahan nyata; field ini opsional di sitemaps.org, jadi tidak ada lebih baik daripada salah. `<changefreq>` + `<priority>` dipertahankan, jumlah `<loc>` tetap 4 sehingga `tests/e2e.spec.ts:55-60` tetap lolos (tidak ada assertion yang menyentuh `<lastmod>`). **Jangan tambahkan kembali** kecuali dibangun mekanisme yang benar-benar mengupdate tanggalnya.

- **Juli 2026 (docs):** README diselaraskan dengan `sitemap.xml` yang sebenarnya. README masih menyebut **6 URL** (termasuk `/about` + `/contact`), padahal file live cuma punya **4 `<loc>`** (`/`, `/jadwal`, `/konser`, `/rumor`) dan `tests/e2e.spec.ts:55-60` justru meng-assert tepat 4, plus `:71-73` meng-assert `/about` & `/contact` **tidak boleh** ada (stub redirect noindex = sinyal soft-404). Jadi dokumennya yang salah, bukan sitemap-nya. Hitungan tes juga dikoreksi: 3 grup, **12 test / 26 assertion**. Kalau nanti sitemap diubah, update angka di README **dan** assertion di `tests/e2e.spec.ts` bersamaan.
...
- **Juni 2026 (feat):** Cookie Consent Banner — `consentBanner` div + `concertid_consent` localStorage key. Terintegrasi GA Consent Mode v2 (`gtag('consent', 'default', {denied})` sebelum GTM, update ke `granted` saat terima). **Update region-scoped (lihat catatan teratas):** default non-EEA `analytics_storage:'granted'`, EEA tetap `denied`.
